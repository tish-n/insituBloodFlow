...
static
svtkUnsignedCharArray *newGhostCellsArray(plb::Box3D domain, int ng, int gnx, int gny, int gnz)
{
...
namespace senseiLP
{
  ...
  struct LPDataAdaptor::DInternals
  {
    ...
    svtkSmartPointer<svtkDoubleArray> AtomPositions;
    svtkSmartPointer<svtkIntArray> AtomTypes;
    svtkSmartPointer<svtkIntArray> AtomIDs;
                                    ...
                                    int nlocal, int **anglelist, int nanglelist)
  {
    ...
    {
      internals.AtomPositions = svtkSmartPointer<svtkDoubleArray>::New();
    }
  ...
  int LPDataAdaptor::GetMeshMetadata(unsigned int id, sensei::MeshMetadataPtr &metadata) 
  {
    ...
    if (id == 0) // id == 0 is cells
    {
      ...
      metadata->MeshType = SVTK_MULTIBLOCK_DATA_SET; //SVTK_POLY_DATA;
  ...
  //----------------------------------------------------------------------
  int LPDataAdaptor::GetMesh(const std::string &meshName, bool structureOnly, svtkDataObject *&mesh)
  {
    int rank, size; 
    MPI_Comm_rank(this->GetCommunicator(), &rank);
    ...
    {  
      ...
      if(!structureOnly)
      {
        ...
        svtkCellArray *Triangles = svtkCellArray::New();

        for (int i = 0 ; i < internals.nanglelist ; i++)
        {
          //cout << rank << " : Triangle Test :" << i << endl;
          svtkTriangle *Triangle = svtkTriangle::New();
    ...
    else if(meshName == "fluid")
    {
      ...
      svtkMultiBlockDataSet *mbfluid = svtkMultiBlockDataSet::New();
      
      //cout << "Inside get mesh " << meshName << endl;

      svtkImageData *FluidImageData = svtkImageData::New();
      FluidImageData->SetDimensions(nlx, nly, nlz); //XXX Changed on 2/24/22 (+2 because of extra layer on each side of svtk file)
  ...
  //----------------------------------------------------------------------
  int LPDataAdaptor::GetMesh(const std::string &meshName, bool structureOnly, svtkCompositeDataSet *&mesh)
  {
    return 0;
  }
  //----------------------------------------------------------------------
  int LPDataAdaptor::AddGhostNodesArray(svtkDataObject* mesh, const std::string &meshName)
  {
   
    return 0;
  }
  //----------------------------------------------------------------------
  int LPDataAdaptor::AddGhostCellsArray(svtkDataObject* mesh, const std::string &meshName)
  {
    ...
    if(meshName == "fluid")
    {
      ...
      svtkImageData *FluidImageData = (svtkImageData*)mbfluid->GetBlock(rank);
      if(!FluidImageData)
      {
      ...
      svtkDataSetAttributes *dsa = FluidImageData->GetAttributes(svtkDataObject::CELL);

      svtkUnsignedCharArray *ga = newGhostCellsArray(this->Internals->domainBox, this->Internals->envelopeWidth , this->Internals->pb_nx, this->Internals->pb_ny, this->Internals->pb_nz);//XXX Fix this to envelopeWidth
      dsa->AddArray(ga);
      ga->Delete(); 
    }
  ...
  //----------------------------------------------------------------------
  int LPDataAdaptor::AddArray(svtkDataObject* mesh, const std::string &meshName,
      int association, const std::string &arrayName)
  {
    ...
    if(meshName == "fluid")
    {
      DInternals& internals = (*this->Internals); 
      /*
      svtkImageData *velocity = dynamic_cast<svtkImageData*>(mesh); //XXX PROBABLY NEEDS TO BE svtkmultiblockdataset
      svtkImageData *vorticity = dynamic_cast<svtkImageData*>(mesh); //XXX extract image data and then add arrays
      svtkImageData *velocityNorm = dynamic_cast<svtkImageData*>(mesh); //XXX Once again, this should be one line, not three
  ...
  //----------------------------------------------------------------------
  int LPDataAdaptor::AddArrays(svtkDataObject* mesh, const std::string &meshName, int association, const std::vector<std::string> &arrayName)
  {
    return 0;
  }
  //----------------------------------------------------------------------
